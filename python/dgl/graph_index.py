...
def _uniform_sampling(gidx, seed_ids, neigh_type, num_hops, expand_factor):
    num_valid_seeds = len(seed_ids)
    if isinstance(expand_factor, list):
        num_neigh_arr = utils.toindex(expand_factor).todgltensor()
    else:
        num_neigh_arr = utils.toindex([expand_factor for i in range(num_hops)]).todgltensor()
    empty_ids = []
    if num_valid_seeds > 1 and num_valid_seeds not in _NEIGHBOR_SAMPLING_APIS.keys():
    ...
    return _NEIGHBOR_SAMPLING_APIS[len(seed_ids)](gidx._handle, *seed_ids, neigh_type,
                                                  num_hops, num_neigh_arr, num_valid_seeds)
