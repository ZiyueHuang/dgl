...
class NodeUpdate(gluon.Block):
    def __init__(self, layer_id, in_feats, out_feats, dropout, activation=None, test=False):
    ...
    def __init__(self,
        ...
        with self.name_scope():
            ...
            for i in range(1, n_layers):
                self.layers.add(NodeUpdate(i, n_hidden, n_hidden, dropout, activation))
            # output layer
...
class GCNInfer(gluon.Block):
    def __init__(self,
        ...
        with self.name_scope():
            ...
            # hidden layers
            for i in range(1, n_layers):
                self.layers.add(NodeUpdate(i, n_hidden, n_hidden, 0, activation, True))
            # output layer
...
def main(args):
    ...
    for i in range(n_layers):
        g.ndata['h_{}'.format(i)] = mx.nd.zeros((features.shape[0], 2*args.n_hidden), ctx=ctx)
    ...
    for epoch in range(args.n_epochs):
        for nf, aux in dgl.contrib.sampling.NeighborSampler(g, args.batch_size, num_neighbors,
            ...
            nf.copy_from_parent(node_embed_names=[['preprocess', 'h_0'], ['agg_h_0']])
            # forward
        ...
        for nf, aux in dgl.contrib.sampling.NeighborSampler(g, args.test_batch_size, g.number_of_nodes(),
                                                            ...
                                                            seed_nodes=test_nid):
            nf.copy_from_parent(node_embed_names=[['preprocess'], ['norm']])
            pred = infer_model(nf)
            batch_nids = nf.layer_parent_nid(-1).as_in_context(ctx)
            batch_labels = labels[batch_nids]
